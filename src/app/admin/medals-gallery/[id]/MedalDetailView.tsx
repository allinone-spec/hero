...
const REVERSE_PATTERN = /reverse|back|verso|rear/i;
const OBVERSE_PATTERN = /obverse|front|recto/i;

/** Try to find a reverse (back) image from wiki images */
function findReverseImage(
  wikiImages: WikiImage[] | undefined,
  mainImageUrl: string
): WikiImage | null {
...
export default function MedalDetailView({
  ...
  // Front: same order as catalog thumb (imageUrl → ribbon → first wiki image)
  const frontUrlRaw = getMedalPrimaryImageUrl(medal);
    ...
    <div className="animate-fade-in-up">
      ...
      >
        <div className="relative px-6 sm:px-10 py-10 sm:py-14 text-center">
          ...
          {/* Medal front & back — side by side */}
          <div className="relative flex items-end justify-center gap-6 sm:gap-10 mb-6">
            {/* Front (obverse) */}
            ...
            <div className="flex flex-col items-center">
              {backUrl ? (
                <SafeWikimediaImg
                  ...
                  alt={`${medal.name} — Back`}
                  className="h-44 sm:h-60 w-auto object-contain drop-shadow-2xl cursor-pointer transition-transform hover:scale-105"
                  onClick={() => setLightboxImg(backUrl)}
                />
              ) : (
                <div
                  className="h-44 sm:h-60 w-32 sm:w-40 rounded-xl flex items-center justify-center border-2 border-dashed"
                  style={{ borderColor: cc.border, color: cc.border }}
                >
                  <span className="text-xs text-center px-3">No reverse image available</span>
                </div>
              )}
              <span
                className="text-[10px] font-bold uppercase tracking-widest mt-2"
